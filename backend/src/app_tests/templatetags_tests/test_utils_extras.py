...
def test_check_get_total_diff_func(stock_price, purchased_price, count, exact):
  user = factories.UserFactory()
  stock = factories.StockFactory(price=stock_price)
  instance = factories.PurchasedStockFactory(user=user, stock=stock, price=purchased_price, count=count)
...
def test_check_is_negative_func(value, exact):
...
def test_check_get_multi_per_pbr_func(per, pbr, exact):
  user = factories.UserFactory()
